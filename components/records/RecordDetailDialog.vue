...
<template>
  <UModal
      ...
      content: 'bg-(--ui-bg)/90 backdrop-blur-[4px] sm:max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl'
    }"
    ...
    <template #content>
      <div v-if="record" class="p-4 sm:p-6">
        <div class="mb-5 flex items-start justify-between gap-4">
          <div>
            <div class="text-sm text-muted">{{ t('recordDetail.title') }}</div>
            <div class="mt-2 flex flex-wrap items-center gap-2">
              ...
              <span class="text-sm text-toned">{{ formatDateTime(record.timeMs) }}</span>
            </div>
          </div>
        ...
        <template v-if="record.type === 'JOIN'">
          <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="rounded-xl border border-default bg-elevated p-4">
              ...
              <div class="mt-2">
                <NuxtLink :to="serverLink(record.server || t('common.unknown'))" class="inline-flex">
        ...
        <template v-else-if="record.type === 'TRANSFER' && transferFlow">
          <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="rounded-xl border border-default bg-elevated p-4">
              <div class="text-sm text-muted">{{ t('recordDetail.transferFlow') }}</div>
              <div class="mt-2 flex flex-wrap items-center gap-2">
        ...
        <template v-else-if="record.type === 'PRIVATE_MESSAGE' && privateServers">
          <div class="rounded-xl border border-default bg-elevated p-4">
            <div class="text-sm text-muted">{{ t('recordDetail.participants') }}</div>
        ...
        <template v-else>
          <div class="grid gap-4 sm:grid-cols-[220px_1fr]">
            <div class="rounded-xl border border-default bg-elevated p-4">
              ...
              <div class="mt-4 text-sm text-muted">{{ t('common.server') }}</div>
              <div class="mt-2">
                <NuxtLink :to="serverLink(record.server || t('common.unknown'))" class="inline-flex">
        ...
        <div class="mt-5 grid gap-4 border-t border-default pt-5 sm:grid-cols-3">
          <div>
            <div class="text-sm text-muted">{{ t('common.time') }}</div>
            <div class="mt-1 text-sm text-highlighted">{{ formatDateTime(record.timeMs) }}</div>
          </div>
          <div>
            <div class="text-sm text-muted">{{ t('common.type') }}</div>
            <div class="mt-1 text-sm text-highlighted">{{ record.type }}</div>
          </div>
          <div v-if="record.command">
            <div class="text-sm text-muted">{{ t('recordDetail.commandSource') }}</div>
